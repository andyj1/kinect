...
// void transformBody(k4abt_body_t &main_body, k4abt_body_t &secondary_body, Mat &R, Mat &T);
// void calculateExtrinsics(k4abt_body_t &main_body, k4abt_body_t &secondary_body, Mat &R, Mat &T);
...
{
    // find average across the row (all joints for each x,y,z)
    ...
    {
        T = cv::Mat(1, 3, CV_32F, Scalar(1, 1, 1));
    }
 ...
 */
void plotBody(k4abt_body_t main_body, k4abt_body_t avg_body, cv::Mat main, k4a::calibration &main_calibration)
{
    ...
    k4a_calibration_intrinsic_parameters_t::_param &i = main_calibration.color_camera_calibration.intrinsics.parameters.param;
    distCoeffs.at<float>(0) = i.k1;
    distCoeffs.at<float>(1) = i.k2;
    distCoeffs.at<float>(2) = i.p1;
    distCoeffs.at<float>(3) = i.p2;
    distCoeffs.at<float>(4) = i.k3;
    distCoeffs.at<float>(5) = i.k4;
    distCoeffs.at<float>(6) = i.k5;
    distCoeffs.at<float>(7) = i.k6;
    ...
    for (auto jointNum : datalist)
    {
        ...
        cv::line(imgList.at(0), jointNum[1], jointNum[0], color, thickness, 8); // 8: cv::line
        // 2: spine chest to spine naval
        cv::line(imgList.at(0), jointNum[2], jointNum[1], color, thickness, 8);
        // 3: neck to spine chest
        cv::line(imgList.at(0), jointNum[3], jointNum[2], color, thickness, 8);
        // 4: clavicle left to spine chest
        cv::line(imgList.at(0), jointNum[4], jointNum[2], color, thickness, 8);
        // 5: shoulder left to clavicle left
        cv::line(imgList.at(0), jointNum[5], jointNum[4], color, thickness, 8);
        // 6: elbow left to shoulder left
        cv::line(imgList.at(0), jointNum[6], jointNum[5], color, thickness, 8);
        // 7: wrist left to elbow left
        cv::line(imgList.at(0), jointNum[7], jointNum[6], color, thickness, 8);
        // 8: hand left to wrist left
        cv::line(imgList.at(0), jointNum[8], jointNum[7], color, thickness, 8);
        // 9: handtip left to hand left
        cv::line(imgList.at(0), jointNum[9], jointNum[8], color, thickness, 8);
        // 10: thumb left to writst left
        cv::line(imgList.at(0), jointNum[10], jointNum[7], color, thickness, 8);
        // 11: clavicle right to spine chest
        cv::line(imgList.at(0), jointNum[11], jointNum[2], color, thickness, 8);
        // 12: shoulder right to clavicle right
        cv::line(imgList.at(0), jointNum[12], jointNum[11], color, thickness, 8);
        // 13: elbow rith to shoulder right
        cv::line(imgList.at(0), jointNum[13], jointNum[12], color, thickness, 8);
        // 14: wrist right to elbow right
        cv::line(imgList.at(0), jointNum[14], jointNum[13], color, thickness, 8);
        // 15: hand right to wrist right
        cv::line(imgList.at(0), jointNum[15], jointNum[14], color, thickness, 8);
        // 16: handtip right to hand right
        cv::line(imgList.at(0), jointNum[16], jointNum[15], color, thickness, 8);
        // 17: thumb right to writst right
        cv::line(imgList.at(0), jointNum[17], jointNum[14], color, thickness, 8);
        // 18: hip left to pelvis
        cv::line(imgList.at(0), jointNum[18], jointNum[0], color, thickness, 8);
        // 19: knee left to hip left
        cv::line(imgList.at(0), jointNum[19], jointNum[18], color, thickness, 8);
        // 20: ankle left to knee left
        cv::line(imgList.at(0), jointNum[20], jointNum[19], color, thickness, 8);
        // 21: foot left to ankle left
        cv::line(imgList.at(0), jointNum[21], jointNum[20], color, thickness, 8);
        // 22: hip right to plevis
        cv::line(imgList.at(0), jointNum[22], jointNum[0], color, thickness, 8);
        // 23: knee right to hip right
        cv::line(imgList.at(0), jointNum[23], jointNum[22], color, thickness, 8);
        // 24: ankle right to hip right
        cv::line(imgList.at(0), jointNum[24], jointNum[23], color, thickness, 8);
        // 25: foot right to ankle right
        cv::line(imgList.at(0), jointNum[25], jointNum[24], color, thickness, 8);
        // 26: head to neck
        cv::line(imgList.at(0), jointNum[26], jointNum[3], color, thickness, 8);
        // 27: nose to head
        cv::line(imgList.at(0), jointNum[27], jointNum[26], color, thickness, 8);
        // 28: eye left to head
        cv::line(imgList.at(0), jointNum[28], jointNum[26], color, thickness, 8);
        // 29: ear left to head
        cv::line(imgList.at(0), jointNum[29], jointNum[26], color, thickness, 8);
        // 30: eye right to head
        cv::line(imgList.at(0), jointNum[30], jointNum[26], color, thickness, 8);
        // 31: ear right to head
        cv::line(imgList.at(0), jointNum[31], jointNum[26], color, thickness, 8);
    ...
    cv::putText(imgList.at(0), "[master device]", cv::Point(30, 30), CV_FONT_HERSHEY_PLAIN, 1.5, COLORS_red, 2, 8, false);
    cv::putText(imgList.at(0), "[synced]", cv::Point(30, 70), CV_FONT_HERSHEY_PLAIN, 1.5, COLORS_green, 2, 8, false);
    cv::imshow("main color camera", imgList.at(0));
    cv::waitKey(1);
}


void plotBody2Dev(std::vector<cv::Point> dataMain, std::vector<cv::Point> dataSecondary, std::vector<cv::Point> dataAvg, cv::Mat main, cv::Mat secondary)
{
    // draws lines between joints for both main and secondary streams
    cv::Scalar color;
    int counter = 0, thickness = 0;
...
void processBodyData(k4abt_body_t main_body, vector<vector<k4abt_body_t>> secondary_body_vector, cv::Mat &main, k4a::calibration main_calibration, vector<int> validSubDevices, vector<cv::Mat> &R, vector<cv::Mat> &T)
{
    ...
    for (int bodyid = 0; bodyid < min_nbody; bodyid++)
    {
        ...
        outfile_avg.close();
        plotBody(main_body, avg_body, main, main_calibration);
    } // end this body ID
...
/*
    calculate extrinsic matrices R, T
*/
void calculateExtrinsics(k4abt_body_t &main_body, k4abt_body_t &secondary_body, Mat &R, Mat &T)
{
...
{
    // main, secondary: [3 x #_joints]
    Mat secondary(3, K4ABT_JOINT_COUNT, CV_32F);
    for (int joint = 0; joint < (int)K4ABT_JOINT_COUNT; joint++)
    {   secondary.at<float>(0, joint) = secondary_body.skeleton.joints[joint].position.v[0];
        secondary.at<float>(1, joint) = secondary_body.skeleton.joints[joint].position.v[1];
        secondary.at<float>(2, joint) = secondary_body.skeleton.joints[joint].position.v[2];
...
void processBody2Dev(k4abt_body_t main_body, k4abt_body_t secondary_body, cv::Mat& main, cv::Mat& secondary, cv::Matx33f main_intrinsic_matrix)
{
    std::vector<cv::Point> dataMain, dataSecondary, dataAvg;
    ...
    plotBody2Dev(dataMain, dataSecondary, dataAvg, main, secondary);

}

void transformBody2Dev(k4abt_body_t& main_body, k4abt_body_t& secondary_body)
{
    // main, secondary: [3 x #_joints]
    Mat main(3,K4ABT_JOINT_COUNT,CV_32F), secondary(3,K4ABT_JOINT_COUNT,CV_32F);
    for (int joint=0; joint < (int)K4ABT_JOINT_COUNT; joint++)
...
/* 
    shows the image
*/
void show(const char *window_title, cv::Mat color_image)
{
    cv::namedWindow(window_title, CV_WINDOW_AUTOSIZE);
    cv::imshow(window_title, color_image);
    cv::waitKey(1);

}

/* 
    draws body joint angles on the RGB image
 */
void plotJointAngles(k4abt_body_t body, cv::Mat &color_image)
{
    // compute joint angles
    ...
    if (joint_angles.size() > 0)
    {
        ...
        for(std::vector<float>::iterator it = joint_angles.begin(); it != joint_angles.end(); it++)
        {
            ...
            std::cerr << "[DEBUG]" << *it*180/M_PI << std::endl;
            
            cv::putText(color_image, angle+": "+to_string(*it*180/M_PI), cv::Point(color_image.cols-200, 30+offset), FONT_HERSHEY_DUPLEX, 1, COLORS_darkorange, 1);
...
void plotJointPositions(std::vector<cv::Point2f> projectedPointsMain, cv::Mat &color_image, std::vector<cv::Point> &dataMain)
{
    for (int i = 0; i < (int)K4ABT_JOINT_COUNT; i++)
    {
        int offsetx = 0;
        ...
        dataMain.push_back(center1);
        cv::Scalar color = COLORS_red;
        cv::circle(color_image, center1, radius, color, CV_FILLED);
        cv::putText(color_image, to_string(i), center1, FONT_HERSHEY_DUPLEX, 1, Scalar(0, 143, 143), 2);
    }
}
void plotJointConnections(std::vector<cv::Point> dataMain, cv::Mat &color_image)
{
    cv::Scalar color;
    int counter = 0, thickness = 0;
    std::list<std::vector<cv::Point>> datalist{dataMain};
    std::vector<cv::Mat> imgList{color_image};
    int imageIdx = 0;
    ...
    for (auto jointNum : datalist)
    {
        ...
        // 1: spine naval to pelvis
        cv::line(imgList.at(imageIdx), jointNum[1], jointNum[0], color, thickness, 8); // 8: cv::line
        // 2: spine chest to spine naval
        cv::line(imgList.at(imageIdx), jointNum[2], jointNum[1], color, thickness, 8);
        // 3: neck to spine chest
        cv::line(imgList.at(imageIdx), jointNum[3], jointNum[2], color, thickness, 8);
        // 4: clavicle left to spine chest
        cv::line(imgList.at(imageIdx), jointNum[4], jointNum[2], color, thickness, 8);
        // 5: shoulder left to clavicle left
        cv::line(imgList.at(imageIdx), jointNum[5], jointNum[4], color, thickness, 8);
        // 6: elbow left to shoulder left
        cv::line(imgList.at(imageIdx), jointNum[6], jointNum[5], color, thickness, 8);
        // 7: wrist left to elbow left
        cv::line(imgList.at(imageIdx), jointNum[7], jointNum[6], color, thickness, 8);
        // 8: hand left to wrist left
        cv::line(imgList.at(imageIdx), jointNum[8], jointNum[7], color, thickness, 8);
        // 9: handtip left to hand left
        cv::line(imgList.at(imageIdx), jointNum[9], jointNum[8], color, thickness, 8);
        // 10: thumb left to writst left
        cv::line(imgList.at(imageIdx), jointNum[10], jointNum[7], color, thickness, 8);
        // 11: clavicle right to spine chest
        cv::line(imgList.at(imageIdx), jointNum[11], jointNum[2], color, thickness, 8);
        // 12: shoulder right to clavicle right
        cv::line(imgList.at(imageIdx), jointNum[12], jointNum[11], color, thickness, 8);
        // 13: elbow rith to shoulder right
        cv::line(imgList.at(imageIdx), jointNum[13], jointNum[12], color, thickness, 8);
        // 14: wrist right to elbow right
        cv::line(imgList.at(imageIdx), jointNum[14], jointNum[13], color, thickness, 8);
        // 15: hand right to wrist right
        cv::line(imgList.at(imageIdx), jointNum[15], jointNum[14], color, thickness, 8);
        // 16: handtip right to hand right
        cv::line(imgList.at(imageIdx), jointNum[16], jointNum[15], color, thickness, 8);
        // 17: thumb right to writst right
        cv::line(imgList.at(imageIdx), jointNum[17], jointNum[14], color, thickness, 8);
        // 18: hip left to pelvis
        cv::line(imgList.at(imageIdx), jointNum[18], jointNum[0], color, thickness, 8);
        // 19: knee left to hip left
        cv::line(imgList.at(imageIdx), jointNum[19], jointNum[18], color, thickness, 8);
        // 20: ankle left to knee left
        cv::line(imgList.at(imageIdx), jointNum[20], jointNum[19], color, thickness, 8);
        // 21: foot left to ankle left
        cv::line(imgList.at(imageIdx), jointNum[21], jointNum[20], color, thickness, 8);
        // 22: hip right to plevis
        cv::line(imgList.at(imageIdx), jointNum[22], jointNum[0], color, thickness, 8);
        // 23: knee right to hip right
        cv::line(imgList.at(imageIdx), jointNum[23], jointNum[22], color, thickness, 8);
        // 24: ankle right to hip right
        cv::line(imgList.at(imageIdx), jointNum[24], jointNum[23], color, thickness, 8);
        // 25: foot right to ankle right
        cv::line(imgList.at(imageIdx), jointNum[25], jointNum[24], color, thickness, 8);
        // 26: head to neck
        cv::line(imgList.at(imageIdx), jointNum[26], jointNum[3], color, thickness, 8);
        // 27: nose to head
        cv::line(imgList.at(imageIdx), jointNum[27], jointNum[26], color, thickness, 8);
        // 28: eye left to head
        cv::line(imgList.at(imageIdx), jointNum[28], jointNum[26], color, thickness, 8);
        // 29: ear left to head
        cv::line(imgList.at(imageIdx), jointNum[29], jointNum[26], color, thickness, 8);
        // 30: eye right to head
        cv::line(imgList.at(imageIdx), jointNum[30], jointNum[26], color, thickness, 8);
        // 31: ear right to head
        cv::line(imgList.at(imageIdx), jointNum[31], jointNum[26], color, thickness, 8);
...
/* 
    draws body joints on the RGB image
 */
void plotJoints(int dev_count, k4abt_body_t main_body, k4abt_body_t avg_body, cv::Mat color_image, k4a::calibration main_calibration)
{
    Mat mainstream(3, K4ABT_JOINT_COUNT, CV_32F), avgstream(3, K4ABT_JOINT_COUNT, CV_32F);
    std::vector<cv::Point3f> main_points, avg_points;
    ...
    if (dev_count == 1)
    {
        ...
        // plot joints with circles
        std::cerr << "plotting joints..." << std::endl;
        std::vector<cv::Point> dataMain;
        plotJointPositions(projectedPointsMain, color_image, dataMain);

        // plot lines connecting the joints
        std::cerr << "connecting joints with lines..." << std::endl;
        plotJointConnections(dataMain, color_image);

        // plot joint angles
        plotJointAngles(main_body, color_image);

        // display image
        const char *window_title = "Single device: RGB";
        show(window_title, color_image);
    }
}

/* 
    wrapper for running system with 1 Kinect
*/
void singleDevice(k4a::device &device, k4a::calibration &sensor_calibration, k4abt::tracker &tracker, ofstream &outfile_orig)
{
    int dev_count = 1;
    int frame_count = 0;
    ...
    while (true)
    {
        ...
        if (device.get_capture(&sensor_capture, std::chrono::milliseconds(K4A_WAIT_INFINITE)))
        {
            ...
            cv::Mat cv_color_image = color_to_opencv(color_image);
                ...
                if (num_bodies > 0)
                {
                    for (size_t i = 0; i < num_bodies; i++)
                    {
                        ...
                        for (int i = 0; i < (int) K4ABT_JOINT_COUNT; i++)
                        {
                            ...
                            // save position, orientation, confidence level information
                            if (outfile_orig.is_open())
                            {
                                outfile_orig << body.id << "," << i << "," << position.v[0] << "," << position.v[1] << "," << position.v[2] << "," << orientation.v[0] << "," << orientation.v[1] << "," << orientation.v[2] << "," << orientation.v[3] << "," << confidenceMap(confidence_level) << "," << endl;
                            }
                        }
                        
                        // show joints
                        int body_count = 1;
                        k4abt_body_t nullbody;
                        plotJoints(dev_count, body, nullbody, cv_color_image, sensor_calibration);
                    }
                }
            }
            else
            {
                //  It should never hit timeout if timeout is K4A_WAIT_INFINITE
...
int twoDevices(vector<k4a::capture> &captures, k4a::calibration &main_calibration, k4a::calibration &secondary_calibration, k4abt::tracker &main_tracker, k4abt::tracker &secondary_tracker, k4abt::frame main_body_frame, vector<k4abt::frame> secondary_body_frame, uint32_t num_bodies_min, bool calculateExtrinsicsBool, cv::Mat &R, cv::Mat &T, ofstream &outfile_orig, ofstream &outfile_sub1)
{
    ...
    try
    {
        ...
        try
        {
            ...
            secondary_body_frame[0] = secondary_tracker.pop_result();
        }
        catch(const std::exception& e)
        {
            std::cerr << e.what() << '\n';
        ...
        if ((main_body_frame != nullptr) && (secondary_body_frame[0] != nullptr))
        {
            secondary_num_bodies = secondary_body_frame[0].get_num_bodies();
        }
        ...
        if (main_num_bodies > 0 && secondary_num_bodies > 0)
        {
            ...
            {
                secondary_body = secondary_body_frame[0].get_body(i);
                ...
                if (main_body.id == secondary_body.id)
                {
                    ...
                    for (int i = 0; i < (int)K4ABT_JOINT_COUNT; i++)
                    {
                        ...
                        {
                            outfile_sub1 << secondary_body.id << "," << i << "," << secondary_position.v[0] << "," << secondary_position.v[1] << "," << secondary_position.v[2] << "," << secondary_orientation.v[0] << "," << secondary_orientation.v[1] << "," << secondary_orientation.v[2] << "," << secondary_orientation.v[3] << "," << confidenceMap(secondary_confidence_level) << "," << std::endl;
                        }
                    }

                    // apply transformation to secondary to convert to main color camera space
                    if (calculateExtrinsicsBool)
                    {
                        calculateExtrinsics(main_body, secondary_body, R, T);
                        calculateExtrinsicsBool = true;
                    }
                    transformBody(main_body, secondary_body, R, T);
                    
                    // printout / display
                    processBody2Dev(main_body, secondary_body, cv_main_color_image, cv_secondary_color_image, main_intrinstic_matrix);
                }
                else
                {
        ...
        {
            std::cerr << "Error! Pop body frame result time out!" << std::endl;
        }
    }
    catch(const std::exception& e)
    {
...
int multipleDevices(size_t num_devices, vector<int> validSubDevices, vector<k4a::capture> &captures, k4a::calibration &main_calibration, k4a::calibration &secondary_calibration, k4abt::tracker &main_tracker, k4abt::tracker &secondary_tracker, k4abt::frame main_body_frame, vector<k4abt::frame> secondary_body_frame, uint32_t num_bodies_min, bool calculateExtrinsicsBool, vector<cv::Mat> R, vector<cv::Mat> T, ofstream &outfile_orig, ofstream &outfile_sub1, ofstream &outfile_sub2)
{
    ...
    std::cerr << "number of available subordinate devices: " << num_devices-1 << std::endl;
    if (num_devices > 1)
    {
        for (size_t sdev = 0; sdev < (int)(num_devices - 1); sdev++)
        {
            ...
            try
            {
                ...
                if (secondary_body_frame[sdev] != nullptr)
                {
                    ...
                    std::cerr << "secondary bodies(dev " << sdev << "): found \t" << secondary_num_bodies << " bodies" << endl;
                }

                // proceed if at least master detected bodies
                if (main_num_bodies > 0)
                {
                    std::cerr << "found in main: "<<main_num_bodies <<"\t in secodary: "<<secondary_num_bodies << std::endl;
                    // possible scenarios
                    ...
                    for (uint32_t body_idx = 0; body_idx < num_bodies_min; body_idx++)
                    {
                        ...
                        else
                        {
                            ...
                            // calculate only at first frame
                            if (calculateExtrinsicsBool)
                            {
                                std::cerr << "Calculating extrinsics "<<std::endl;
                                calculateExtrinsics(main_body, secondary_body_vector[sdev][body_idx], R.at(sdev), T.at(sdev));
                                calculateExtrinsicsBool = false;
                            }

                            // transform sub to master and store directly
                            std::cerr << "Entering transform body function" << std::endl;
                            transformBody(main_body, secondary_body_vector[sdev][body_idx], R.at(sdev), T.at(sdev));
                            std::cerr << "completed transform body function" << std::endl;

                            // store joints data for each body in csv
                            for (int i = 0; i < (int)K4ABT_JOINT_COUNT; i++)
                            {
    ...
    if (main_num_bodies > 0)
        processBodyData(main_body, secondary_body_vector, cv_main_color_image, main_calibration, validSubDevices, R, T);
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    // MultiDeviceCapturer finds the master based on the sync mode, and the device index may not necessarily be in order
    vector<uint32_t> device_indices;
    num_devices = static_cast<size_t>(atoi(argv[1]));
    for (int i = 0; i < k4a::device::get_installed_count(); i++) device_indices.push_back(i);
    ...
    */
    ...
    // output file streams to store CSV's
    ...
    */
    ...
    {        
		...
		device.start_cameras(&main_config);
		k4a::calibration sensor_calibration = device.get_calibration(main_config.depth_mode, main_config.color_resolution);
		k4abt::tracker tracker = k4abt::tracker::create(sensor_calibration);

        singleDevice(device, sensor_calibration, tracker, outfile_orig);
    ...
    */    
        ...
        /* 
            configure for multi device capture
        */
        ...
        // ============ initialize body tracker (same for secondaries since config is the same; this is to speed up)
        ...
        uint32_t num_bodies_min = 1;

        /* 
            2 Devices
        */
        bool calculateExtrinsicsBool = true;
        if (num_devices == 2) {
           cv::Mat R = cv::Mat::eye(3,3,CV_32F);
           cv::Mat T = cv::Mat(1, 3, CV_32F, Scalar(1, 1, 1)); // extrinsics
            while(true)
            {    
                captures = capturer.get_synchronized_captures(secondary_config, true);
                int res = twoDevices(captures, main_calibration, secondary_calibration, main_tracker, secondary_tracker, main_body_frame, secondary_body_frame, num_bodies_min, calculateExtrinsicsBool, R, T, outfile_orig, outfile_sub1);
                if (res == 0) break;
            }
        ...
        */
            ...
            vector<int> validSubDevices;
            vector<cv::Mat> R, T(num_devices); // extrinsics
            for (int i = 0; i < num_devices; i++)
            {
                R.push_back(cv::Mat::eye(3,3,CV_32F));
                T.push_back(cv::Mat(1, 3, CV_32F, Scalar(1, 1, 1)));
            }
            ...
            while (true)
            {
                ...
                captures = capturer.get_synchronized_captures(secondary_config, true);
                int res = multipleDevices(num_devices, validSubDevices, captures, main_calibration, secondary_calibration, main_tracker, secondary_tracker, main_body_frame, secondary_body_frame, num_bodies_min, calculateExtrinsicsBool, R, T, outfile_orig, outfile_sub1, outfile_sub2);
                if (res > 0) break;
            }   // ========== END while loop
        }   // END main function
    }
