...
static void log_lagging_time(string lagger, k4a::capture &master, k4a::capture &sub)
{
            ...
            for (size_t i = 0; i < subordinate_devices.size(); ++i)
            {
                ...
                if (master_color_image && sub_image)
                {
                    ...
                    if (sub_image_time_error < -MAX_ALLOWABLE_TIME_OFFSET_ERROR_FOR_IMAGE_TIMESTAMP)
                    {
                        ...
                        string sub_name = "sub";
                        sub_name.append(": ").append(to_string(i));
                        log_lagging_time(sub_name, captures[0], captures[i + 1]);
